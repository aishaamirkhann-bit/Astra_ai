...
def authorize_consent(payload: ConsentAuthorizationRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if payload.order_ref:
        order = db.query(Order).filter(Order.order_ref == payload.order_ref, Order.user_id == current_user.id).first()
        is_cart_checkout = payload.order_ref.startswith(f"CART-{current_user.id}-")
        if not order and not is_cart_checkout:
            raise HTTPException(status_code=404, detail="Order not found")
        if order and abs(order.price - payload.amount) > 0.01:
    ...
    if payload.auth_method == "Voice":
        ...
        consent = FinancialConsentLog(user_id=current_user.id, amount=payload.amount, auth_method="Voice", voice_transcript=payload.voice_transcript, status="Approved", reference_order_id=order.id if order else None)
    ...
    if not payload.consent_id:
        ...
        consent = FinancialConsentLog(user_id=current_user.id, amount=payload.amount, auth_method="OTP", status="Flagged", reference_order_id=order.id if order else None, otp_code_hash=hash_otp(code), otp_expires_at=datetime.now(timezone.utc) + timedelta(minutes=5))
        db.add(consent); db.commit(); db.refresh(consent)
    ...
    consent = db.query(FinancialConsentLog).filter(FinancialConsentLog.consent_id == payload.consent_id, FinancialConsentLog.user_id == current_user.id, FinancialConsentLog.auth_method == "OTP").first()
    ...
    consent.status = "Approved"; db.commit()
