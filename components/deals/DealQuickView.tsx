...
import { CheckCircle2, Minus, Plus, ShieldCheck, ShoppingBag, Sparkles, X, Zap } from "lucide-react";
            ...
            <div className="grid gap-7 p-5 lg:grid-cols-[1.05fr_.95fr] lg:p-7">
              ...
              <div className="lg:sticky lg:top-24 lg:self-start">
                ...
                <div className="mt-7 space-y-3">
                  <button type="button" disabled={busyAction !== null} onClick={() => onAction(deal, "reserve", { quantity, size, color })} className="flex w-full items-center justify-center gap-2 rounded-xl bg-astra-gradient px-4 py-3.5 text-sm font-bold text-white shadow-glow disabled:opacity-60"><Zap className="h-4 w-4" /> {busyAction === "reserve" ? "Reserving deal…" : "Direct Checkout / Buy Now"}</button>
                  <button type="button" disabled={busyAction !== null} onClick={() => onAction(deal, "cart", { quantity, size, color })} className="flex w-full items-center justify-center gap-2 rounded-xl border border-base-600 bg-base-800 px-4 py-3 text-sm font-semibold text-ink-100 hover:border-astra-indigo disabled:opacity-60"><ShoppingBag className="h-4 w-4" /> {busyAction === "cart" ? "Adding…" : "Add to Wallet / Cart"}</button>
                </div>
