...
def test_create_update_and_match_verified_goal(auth_client) -> None:
    ...
    created = auth_client.post("/api/v1/goals/create", json={
        "target_title": "Verified Tech Deal Test", "target_price": 1000000,
        "category": "Tech", "priority_level": "High", "deadline": "2027-12-31",
    })
    assert created.status_code == 201
    goal = created.json()
    updated = auth_client.put(f"/api/v1/goals/{goal['goal_id']}/update", json={"deposit_amount": 100, "target_price": 900000})
    assert updated.status_code == 200
    assert updated.json()["saved_amount"] == 100
    assert updated.json()["target_price"] == 900000

    matches = auth_client.get("/api/v1/goals/matched-deals")
...
def test_budget_guardrail_creates_warning_and_installment(auth_client) -> None:
    original = auth_client.get("/api/v1/goals/budget").json()["budget"]
    created = auth_client.post("/api/v1/goals/create", json={
        "target_title": "Guardrail Test", "target_price": 1000000,
        "category": "Tech", "priority_level": "Medium",
    }).json()
    auth_client.put("/api/v1/goals/budget", json={"monthly_limit": 1000, "current_spent": 900})
    matches = auth_client.get("/api/v1/goals/matched-deals").json()
    ...
    auth_client.put("/api/v1/goals/budget", json={
...
def test_repeated_match_scan_deduplicates_alerts(auth_client) -> None:
    first = auth_client.get("/api/v1/goals/matched-deals")
    second = auth_client.get("/api/v1/goals/matched-deals")
